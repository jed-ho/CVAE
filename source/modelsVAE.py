...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        # Flatten the input using reshape instead of view
        x = x.reshape(x.size(0), -1)
        # Pass through fully connected layers with ReLU activation
        for layer in self.fc_layers:
            x = F.relu(layer(x))
        # Perform reparameterization to get latent variables
...
class Decoder(nn.Module):
    ...
    def forward(self, z):
        ...
        # Reshape back to original input shape using reshape
        x = x.reshape(-1, self.num_param, self.window_size)
        return x

class VAE(nn.Module):
    """
    VAE model with fully connected architecture
    """
    def __init__(self, latent_dim, num_param, window_size, scale_flag):
    ...
    def forward(self, x):
        # Encode input and sample from latent space
